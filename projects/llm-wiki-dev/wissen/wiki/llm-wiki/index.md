# LLM Wiki Topic Index

Compiled topic index for `llm-wiki` sources.

## Concepts

- [llm-wiki](./concepts/llm-wiki.md) - core concept for the topic and anchor page for the persistent wiki pattern.
- [Persistent Wiki Pattern](./concepts/persistent-wiki-pattern.md) - core idea: compile and maintain knowledge instead of re-deriving via query-time RAG.
- [Operation Loop](./concepts/operation-loop.md) - ingest/query/lint/audit-style loops that keep the knowledge base current.
- [Runtime and Skill Packaging](./concepts/runtime-and-skill-packaging.md) - portable skill and plugin distribution across agent runtimes.
- [Local Search and Retrieval Tooling](./concepts/local-search-and-retrieval-tooling.md) - local search engines (for example qmd) and MCP integration for scale.

## Connections

- [Implementations Comparison](./connections/implementations-comparison.md) - relationship between the Karpathy idea file and community implementations.
- [LLM Wiki and Docling Ingestion Boundary](./connections/llm-wiki-and-docling-ingestion-boundary.md) - cross-topic bridge between source parsing and persistent wiki compilation.

## References

- `raw/karpathy-llm-wiki-gist.md`
- `raw/karpathy-llm-wiki-raw-idea-file.md`
- `raw/lewislulu-llm-wiki-skill.md`
- `raw/nvk-llm-wiki.md`
- `raw/opencode-skills-docs.md`
- `raw/qmd-local-markdown-search.md`
