...
\subsection{Ausgangslage}\label{ausgangslage-2}
...
auch hohe Performanz im Schreiben von Daten.\footnote{www.opencode.de/URL-To-Be-Announced.}
...
Daneben wächst der Einsatz von R stetig mit aktuell 200 aktiven
Nutzenden im StBA und FDZ stetig.

Empfehlung des FDSZ der deutschen Bundesbank.
(\textcite{gomolka2021largedata}) hat den spezialfall der in-memory
Auswertung betrachtet mit den folgenden Ergebnissen:

-R zeigt große Performancevorteile gegenüber Stata für FDZ Aufgaben - Zu
Beachten: Optimierung der Gesamtzeit (d.h. Rechenzeit +
\textbf{Entwicklungszeit}) - benutzt \{parquet\} bereits Standardmäßig
...
\subsection{Möglichkeiten in R}\label{muxf6glichkeiten-in-r}
...
\tightlist
\item
  Ökosystem von Packages zur Datenverarbeitung. bekannt für besonders
  hohe Nutzerfreundlichkeit und einer eigenen Programmiersyntax sog.
  \{tidy\}-Syntax, die den Anspruch hat menschlicher Sprache abzubilden:
\end{itemize}
...
Performamz kommen.\footnote{Lediglich in der
  Arbeitsspeicherbeanspruchung, ist SAS deutlich besser als klassische
  R-Methoden. Gegenüber hoch-performanten R-Methoden benötigt SAS jedoch
  den 10-fachen Arbeitsspeicher.}

beachte/erwähne:
https://www.r-bloggers.com/2024/04/the-truth-about-tidy-wrappers-3/

\subsection{Ausgewählte Best Practices in
R}\label{ausgewuxe4hlte-best-practices-in-r}
...
\tightlist
\item
  \emph{Variablenselektion}: Auswahl der benötigten Variablen in der
  Einlesefunkktion ist wesentliche Best-Practices (ab mittleren
  Datenmengen, besonders für klassiche Methoden). Eine
  Variablenselektion erst nach dem Einlesen verursacht unnötige
  Wartezeiten, Ressourcenbeanspruchung und Abbrüche
\item
  ...
  mit der mehrfachen Variablenselektion. Die folgenden Hoch-performante
  Technologien, kommen auch flexibel aus ohne manuelle Selektion
\end{itemize}

\subsection{Hoch-performante Technologien in
R}\label{hoch-performante-technologien-in-r}
...
\tightlist
\item
  Einrichtbarkeit neuer Package etc. organisatorisch eingeschränkt und
  verzögert (S. Abschnitt Kapitel~\ref{sec-infrastruktur}).
\item
...
\item
  mit unter \textbf{300 MB Arbeitsspeicher} -- \{duckdb\} und \{polars\}
...
verfügbaren 512 GB Arbeitsspeicher vollständig beansprucht. Wir
klassische Methoden ist somit deutlich mehr Erfahrung der Nutzenden
notwendig, um die relevanten Variablen manuell geschickt zu selektieren
unter Beobachtung des Arbeitsspeicherverbrauchs.

Insgesamt kommen wir zu dem Ergebnis, dass \{arrow\} kombiniert mit
\{parquet\} im Gesamtpaket die beste Datenverarbeitungsmethode für die
Bewältigung unserer Aufgaben aus den folgenden Gründen ist:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Für die relevanteste Datenmenge zeigte \{arrow\} die geringsten
  Laufzeiten unter 5 Sekunden, was der Fachstatistik Antworten auf
  Sonderanfragen in Echtzeit ermöglicht und der Wissenschaft alle
  denkenswerten explorativen Analysen.
\item
  auch die Nutzererfahrung war mit \{arrow\} am besten und die damit
  einhergehenden Entwicklungaufwände waren dadurch am geringsten.
\item
  die anschauliche \{tidyverse\} Syntax lässt sich zum Großteil
  änderungsfrei und ohne Performanzverluste auf \{arrow\} übertragen.
  \{duckdb\} kann bei Kenntnissen in SQL zu bevorzugen sein und
  \{polars\} mit Erfahrungen in Python.
\item
...
\item
  ...
  sind, haben wir bisher immer eine Lösung gefunden, um die gewünschte
  Auswertung umzusetzen, bspw. durch Kombination mit \{duckdb\}.
\item
...
\subsection{(Weitere Erkenntnisse des
Experiments)}\label{weitere-erkenntnisse-des-experiments}
