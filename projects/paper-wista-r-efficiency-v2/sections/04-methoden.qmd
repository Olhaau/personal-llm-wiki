...
Empfehlung des FDSZ der deutschen Bundesbank. (@gomolka2021largedata) hat den spezialfall der in-memory Auswertung betrachtet mit den folgenden Ergebnissen:

-R zeigt große Performancevorteile gegenüber Stata für FDZ Aufgaben
- Zu Beachten: Optimierung der Gesamtzeit (d.h. Rechenzeit + **Entwicklungszeit**)
- benutzt {parquet} bereits Standardmäßig
...
### Ausgewählte Best Practices in R 

Ab mittleren Datenmengen (abhängig von CPU und RAM) sind Best Practices:

- *Variablenselektion*: Auswahl der benötigten Variablen in der Einlesefunkktion ist wesentliche Best-Practices (ab mittleren Datenmengen, besonders für klassiche Methoden). Eine Variablenselektion erst nach dem Einlesen verursacht unnötige Wartezeiten, Ressourcenbeanspruchung und Abbrüche
- *Datenaufteilung*: Wenn der Arbeitsspeicher nicht ausreicht, um die benötigten Daten einzulesen, bietet eine Aufteilung auf mehrere RAM-Gerechte Datenpakete helfen. Für klassische Methoden kombiniert mit der mehrfachen Variablenselektion. Die folgenden Hoch-performante Technologien, kommen auch flexibel aus ohne manuelle Selektion

### Hoch-performante Technologien in R
...
- Einrichtbarkeit neuer Package etc. organisatorisch eingeschränkt und verzögert (S. Abschnitt @sec-infrastruktur).
