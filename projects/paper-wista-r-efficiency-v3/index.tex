...
\setstretch{1}
\renewcommand{\totalchars}{36 929}
\renewcommand{\totalwords}{4 488}
...
Zur Bewältigung der technischen Limitationen in der Datenverarbeitung
konnten im Jahr 2025 bedeutende Fortschritte beim Ausbau der
analytischen Infrastruktur des Statistischen Bundesamts und des FDZ
erreicht werden (s. Absatz~\ref{sec-infrastruktur}). Für die
Statistikproduktion konnte im Januar 2025 ein R- und Python-Server um
erhebliche Rechenkapazitäten erweitert werden. Vorangetrieben durch das
NeSt steht seit November 2025 im FDZ ein \textbf{OnSite-Server} mit
ausgeweiteten Rechenkapazitäten für Analysen in R und Stata exklusiv für
GWAP- und KDFV-Nutzungen zur Verfügung.
...
handelt, erweitert eine große internationale Gemeinschaft Funktionalität
von R für hohe Performanz oder Nutzerfreundlichkeit (s.
Absatz~\ref{sec-methoden}). Dieser Artikel wird potenzielle R-Pakete für
den Einsatz im Statistischen Bundesamt und FDZ bewerten (s.
Absatz~\ref{sec-benchmark}) und anhand des BTP demonstrieren, wie
relevante Auswertungen nachvollziehbar innerhalb von wenigen Sekunden
durchgeführt werden können (s. Absatz~\ref{sec-ergebnis}).
...
Beobachtungen auf uca. 78 Millionen erhöhen. Auch der Merkmalsumfang von
...
erstellt. Sowohl bei den Standard- also auch Sonderauswertungen werden
...
Bundesamts ist die kommerzielle Analysesoftware \textbf{SAS 9.4}, das
auf dedizierte Server bereitgestellt wird.\footnote{Da SAS bereits
  ...
  verfügen. SAS wurde in 2025 auf Version 9.4M8 aktualisiert.} SAS
...
müssen, und nur ausgewählte Datenverabeitungsschritte können von Stata
parallelisiert werden. Durch die manuelle \emph{Variablenauswahl} lassen
...
Die \textbf{Statistiksoftware R} ist bekannt als native
Programmiersprache in der Statistik und in Datenwissenschaften. Dadurch
weist R eine hohe Anwenderorientierung und fokussiert sich auf
...
analysieren.\footnote{Das Schreiben von Daten wird nicht vertieft
  ...
  sind. Die genannten Lösungen sind jdennoch beim Schreiben effizient.
...
\tightlist
\item
  \textbf{Variablenselektion}: Beim Einlesen von DatenAuswahl der
  ...
  kann zu Abbrüchen führen. Auch Stata profitiert Beschleuniging von
  39.9\% in R und 33.9\% in \textcite{gomolka2021largedata}.
\item
...
\item
  ...
  Geschwind igkeit bei gleichzeitig sparsamer Nutzung der
  Systemressourcen.
\end{itemize}
...
systematisher Performanzvergleich wird im FDZ-OnSite-Server für die
Anwendungsfälle aus Absatz~\ref{sec-btp} im folgenden
...
Performanzmessungen auch in für 1\% des simulierten BTP-Daten (etwa.
...
der Arbeitsspeicher um ein tausensfaches reduzieren.
