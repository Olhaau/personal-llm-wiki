...
\setstretch{1}
\renewcommand{\totalchars}{39 612}
\renewcommand{\totalwords}{4 797}
...
\renewcommand{\footnotecountnum}{15}
...
\newcommand{\sectionrows}{            Einleitung & \textcolor{wistaBlue}{\textbf{1 231}} & \textcolor{wistaBlue}{\textbf{12 828}} & \textcolor{wistaBlue}{\textbf{5}} & \textcolor{wistaBlue}{\textbf{7}} & --- \\
            Anwendungsfall Business-Tax-Panel & \textcolor{wistaBlue}{\textbf{125}} & \textcolor{wistaBlue}{\textbf{1 302}} & \textcolor{wistaBlue}{\textbf{0}} & \textcolor{wistaBlue}{\textbf{1}} & --- \\
            Techniche Infrastruktur & \textcolor{wistaBlue}{\textbf{145}} & \textcolor{wistaBlue}{\textbf{1 511}} & \textcolor{wistaBlue}{\textbf{1}} & \textcolor{wistaBlue}{\textbf{1}} & --- \\
            Datenverarbeitungsmethoden & \textcolor{wistaBlue}{\textbf{810}} & \textcolor{wistaBlue}{\textbf{8 441}} & \textcolor{wistaBlue}{\textbf{3}} & \textcolor{wistaBlue}{\textbf{5}} & --- \\
            Performanzvergleich & \textcolor{wistaBlue}{\textbf{999}} & \textcolor{wistaBlue}{\textbf{10 411}} & \textcolor{wistaBlue}{\textbf{4}} & \textcolor{wistaBlue}{\textbf{6}} & --- \\
            Ergebnis für das vollständige BTP & \textcolor{wistaBlue}{\textbf{84}} & \textcolor{wistaBlue}{\textbf{875}} & \textcolor{wistaBlue}{\textbf{0}} & \textcolor{wistaBlue}{\textbf{1}} & --- \\
            Fazit & \textcolor{wistaBlue}{\textbf{407}} & \textcolor{wistaBlue}{\textbf{4 241}} & \textcolor{wistaBlue}{\textbf{2}} & \textcolor{wistaBlue}{\textbf{2}} & --- \\
}
...
\item
  ...
  BTP mit hohem Analysepotenzial (S. \textbf{?@sec-btp})
\item
...
analysieren.\footnote{Das Schreiben von Daten wird in unserer
  ...
  betrachtung von Datenbanken und Spark. Die Werkzeuge sind auch in
  Programmiersprachen wie Python verfügbar. Dort kann gibt es weitere
  Möglichkeiten für hohe Perfomanz. Wir fokussieren uns auf den standard
  R-Nutzenden.}
...
\item
  ...
  Codes ausgelegt ist. Eine eigene Programmiersyntax
  (\emph{dplyr-Syntax}) orientiert sich an menschlicher Sprache und
  macht den Code auch für Einsteiger gut verständlich. Diese
...
beschriebene Variablenselektion eine oft notwendige \textbf{Best
Practice}, die direkt spürbare Performanzgewinne birgt. Darüberhinaus
kann die Datenaufteilung kombiniert mit sog. Lazy-Computation
gewinnbringend sein:
...
Das Datenformat Apache
\textbf{\href{https://parquet.apache.org/}{Parquet}} ist ein
spaltenbasiertes, komprimiertes Datenformat, das wenig
Festplattenspeicher beansprucht und für Analyse großer Datensätze
entwickelt wurde. Es reduziert Lese-/Schreibzeiten deutlich, da
effizient auf einzelne Spalten zugegriffen werden kann. In einer
Untersuchung des Forschungsdaten- und Servicezentrum (FDSZ) der
Bundesbank verkürzte die Nutzung von Parquet in R Rechenzeiten im
Vergleich zu optimiertem Code in Stata um 65 \% (von 77,6 Sek. auf 27,1
Sek.) (\textcite{gomolka2021largedata}). Sodass das FDSZ Parquet-Dateien
bereits standardmäßig bereitstellt.

Für hohe performante Verarbeitung von Datenmengen überhalb des RAMs
bieten die folgenden \textbf{R-Paketen} entscheidene Vorteile:

\begin{itemize}
\tightlist
\item
  \textbf{\href{https://arrow.apache.org/docs/r/index.html}{arrow}}
  \autocite{R-arrow}: Ein C++-basiertes R-Paket, das speziell für die
  Arbeit mit großen Datensätzen entwickelt wurde. Es ermöglicht die
  Verarbeitung von Daten, die größer als der verfügbare Arbeitsspeicher
  sind, durch intelligente Datenaufteilung und Parallelisierung. Arrow
  ist optimal auf das Parquet-Dateiformat abgestimmt und kann
  Berechnungen automatisch optimieren (Lazy-Computation). Damit ist
  Arrow besonders geeignet für Datensätze, die mehrere Gigabyte oder
  sogar Terabyte umfassen. Die gewohnte R-Syntax (dplyr) bleibt dabei
  erhalten, was die Nutzung für R-Anwender/-innen besonders einfach
  macht. Das Paket ergänzt die Features von \{data.table\} und kann in
  bestimmten Szenarien in-memory effizienter sein.
\item
  \textbf{\href{https://duckdb.org/docs/stable/clients/r}{duckdb}}
  \autocite{R-duckdb}: Ein eingebettetes Datenbanksystem, das ohne
  separaten Datenbankserver direkt in R genutzt werden kann. Es
  verarbeitet Daten spaltenweise, was besonders bei analytischen
  Abfragen auf großen Datensätzen sehr schnell ist. DuckDB nutzt die
  bekannte SQL-Syntax für Datenabfragen und kann nahtlos mit Arrow-Daten
  zusammenarbeiten. Das System verwaltet den Arbeitsspeicher intelligent
  und kann auch Daten verarbeiten, die größer als der verfügbare RAM
  sind.
\item
  \textbf{\href{https://pola-rs.github.io/r-polars/}{polars}}
  \autocite{R-polars}: Ein Rust-basiertes R-Paket, das durch die
  Programmiersprache Rust besonders effizient arbeitet. Es bietet sowohl
  eine Python-orientierte Syntax\footnote{Cookbook für den Einstieg in
    Polars: https://ddotta.github.io/cookbook-rpolars/first\_steps.html}
  als auch eine dplyr-kompatible Variante (tidypolars), die für
  R-Nutzende vertrauter ist und die Performanz nur geringfügig
  beeinträchtigt. Polars nutzt moderne Optimierungstechniken und ist für
  sehr große Datensätze ausgelegt. Die Rust-Implementierung sorgt für
  hohe Geschwindigkeit bei gleichzeitig sparsamer Nutzung der
  Systemressourcen.
\end{itemize}

\subsection{Gültigkeit von externen
Benchmarks}\label{guxfcltigkeit-von-externen-benchmarks}
...
Die auf den Servern des StBA und FDZ verfügbaren
\textbf{Packageversionen} lassen sich aus organisatorischen Gründen
nicht flexibel ändern, da die Genehmigung und Einrichtung neuer Packages
nur auf Antrag durch unser R-Betriebsteam erfolgt (i.d.R. innerhalb von
1-2 Tagen ). Die Package-Versionen, die zum Zeitpunkt des in
Kapitel~\ref{sec-benchmark} dargestellten Performanz-Vergleiches
verfügab waren, können Tabelle Tabelle~\ref{tbl-pkg-versions} entnommen
werden. Ultimatives Optimum nicht wichtig, langzeitig stabil gute
Performanz ist entscheidend.
