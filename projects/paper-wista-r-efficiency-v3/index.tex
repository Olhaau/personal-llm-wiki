...
\setstretch{1}
\renewcommand{\totalchars}{37 035}
\renewcommand{\totalwords}{4 506}
...
Herausforderungen zu bewältigen. Geeignete Werkzeuge können leicht
...
Die FDZ-Infrastruktur muss daher in der Lage sein, um umfangreiche
...
handelt, erweitert eine große internationale Gemeinschaft daran, R in
Form von sog. R-Paketen um diverse Funktionalität hoher-performanz oder
Nutzerfreundlichkeit zu erweitern (s. Absatz~\ref{sec-methoden}). Dieser
Artikel wird potenzielle R-Pakete für den Einsatz im Statistischen
Bundesamt und FDZ bewerten (s. Absatz~\ref{sec-benchmark}) und anhand
des BTP demonstrieren, wie relevante Auswertungen nachvollziehbar
innerhalb von wenigen Sekunden durchgeführt werden können (s.
...
Für die vielfältigen Analysewünsche der Forschenden, der damit
einhergenden Dokumentation des Datensatzes in den Metadatenreports
\autocite{fdz2024btp} des FDZ, und den Auswertungsbedarf des
Fachbereiches ist der Datensatz möglichst wenig beschränkt worden und
dadurch ressourcenintensive. Anstelle einer Stichprobe wurden die
...
Beobachtungen auf ca. 78 Millionen erhöhen. Auch der Merkmalsumfang von
...
erstellt. Sowohl bei den Standard- als auch Sonderauswertungen werden
meist spezifische Anforderungen angegeben. Dafür sind verschiedene
Bearbeitungsschritte in Form von Filterungen der Beobachtungen (z.B.
Angaben nur für Organträger) und Auswahl von Merkmalen sowie
anschließende Berechnungen nötig. Abschließend wird die Tabelle in die
vorgegebene Veröffentlichungsform gebracht. Das FDZ hat für die
Erstellung des Metadatenreports einen ähnlichen Bedarf. Um den Bedarf
nach deskriptiven Auswertungen abzubilden wird die Tabelle zur
...
begrenzend. Bisher waren die GWAP auf die \emph{Single-Core-Version von
Stata mit begrenztem RAM} beschränkt, wodurch viele FDZ-Produkte ohne
Datenzuschnitt am GWAP nicht bearbeitet werden konnten.
...
GWAP- bzw. KDFV-Nutzung im FDZ sicherzustellen, sollte die Systeme kurze
Rechenzeiten auf dem aktuellen Stand der Technik ermöglichen -- und
idealerweise Auswertungen großer Datenmengen in Echtzeit (d.h. innerhalb
...
es stehen zahlreiche Schulungsmaterialien zur Verfügung. Viele
...
sich R auch an steigender Beliebtheit in der amtlichen Statistik, wie
der Wachstum der Gemeinschaft ``The Use of R in Official Statistics''
...
Workbench}, da sie wesentliche Funktionen für kollaborativen Arbeiten,
Skalierbarkeit bietet und den Betrieb der Systeme unterstützt.
...
Datenverarbeitungswerkzeuge, für Einsteiger oder in stark abgeschotteten
Infrastrukturen zweckdienlich sind. Base R ist heutzutage jedoch weder
in Benutzerfreundlichkeit, noch in Performanz zeitgemäß. Diese Lücke
wird durch eine Vielzahl von R-Paketen geschlossen, wie die folgenden
hervorragenden R-Pakete zeigen.

\begin{itemize}
\tightlist
\item
  \textbf{data.table} \autocite{R-data.table}: Ein ausgereiftes R-Paket,
  das Datenverarbeitung deutlich schneller macht als
...
\item
  \textbf{Lazy-Computation}: Ohne das Nutzende ihren Code verändern
  ...
  seinen Code ändern muss. z.\,B. durch Lazy-Computation{[}\^{}lazy{]}
  -- ohne dass eine manuelle Variablenauswahl erforderlich ist (siehe
  Absatz~\ref{sec-methoden}).
\end{itemize}
...
den Speicherbedarf erheblich. (\textcite{gomolka2021largedata}) zeigte
im FDZ-Kontext massive Geschwindigkeitsgewinne anhand Parquet, sodass im
FDSZ bereits standardmäßig Parquet-Dateien angeboten werden.
