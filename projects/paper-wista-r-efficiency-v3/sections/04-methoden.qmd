...
- Variablenselektion, e.g in BBK: Bot Beschleuniging von 39.9% in R und 33.9% in Stata
...
z. B. durch Lazy-Computation[^lazy] – ohne dass eine manuelle Variablenauswahl erforderlich ist (siehe @sec-methoden).
...
[^lazy]: Ohne das Nutzende ihren Code verändern müssen, speichert die Lazy-Computation geplante Datenverarbeitungsschritte im Hintergrund, ohne sie sofort auszufüren. Erst wenn ein konkretes Ergebnis abgefragt wird, führt das System die Berechnung durch. Dadurch werden nur die tatsächlich benötigten Daten verarbeitet und unnötige Schritte vermieden – ohne dass der Nutzende seinen Code ändern muss.

Diverse benchmarks kommen zu unterschiedlichen Ergebnissen, daher ist die untersuchung in eigener Infrastruktur mir relevanten Use Cases wichtig und wird im folgenden @sec-benchmark durchgeführt.[^bm-dt][^bm-ddb][^bm-polars]
...
- R-package [@R-arrow], C++ basiert, erlaubt Parallelisierung, Datenaufteilung, Lazy-Computations, Datenformat im RAM passend zu Parquet
- tba - s. Vortrag

#### DuckDB

- R-Package [@R-duckdb], in-memory DB, ...
- tba - s. Vortrag
