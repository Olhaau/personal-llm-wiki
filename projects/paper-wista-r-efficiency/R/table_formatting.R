...
format_wista_table <- function(data,
  ...
  # For non-LaTeX (including DOCX), use simple kable
  if (!is_latex) {
    # Use col_names if provided
    if (!is.null(col_names)) {
      col_names_to_use <- col_names
    } else {
      col_names_to_use <- names(data)
    }
    
    # Create a simple kable - knitr will auto-detect the right format
    # Don't convert to string - return the kable object so it can be properly rendered
    tbl <- knitr::kable(
      data,
      col.names = col_names_to_use,
      align = align,
      caption = caption,
      row.names = FALSE
    )
    
    # Return the kable object directly (not as.character)
    # This allows knitr/pandoc to convert it properly
    return(tbl)
  }
...
#' For DOCX: returns the kable object for proper rendering
#' 
#' @param tbl_out Table output from format_*_table functions
#' 
#' @return For DOCX: kable object; For LaTeX: NULL (after cat)
output_table <- function(tbl_out) {
  if (is.character(tbl_out)) {
    ...
    # DOCX/other - return the kable object so it renders
    return(tbl_out)
  }
}
