...
Die Verbreitung von R steigt.

### Anforderungen an Datenverarbeitungsmethoden

- Performanz und Nutzererfahrung auf dem aktuellen Stand der Technik
- leicht einzurichten, d.h. wenig Abhängigkeiten und unabhängig von Internet bzw. Cloud betreibbar
- langfristig stabil und gut unterstützt
- leicht nutzbar, gut dokumentiert, schulbar

### Datenverarbeitungsmethoden in R
...
#### Klassische Datenverarbeitungsmethoden
...
So beliebt, dass die Community sie vielen anderen Packages ergänzt hat. Bspw. für {data.table} die Packages {dtplyr} oder {tidytable}. Wie im Falle von {dtplyr} kann es dabei aber zu Einschränkung der Performamz kommen.[^bm-tidy]
...
von Desktopanwendung unterm Tisch zi Performamten Analyseserver paradigmenwechsel notwendig: in RAM vs nicht im RAM!
...
tbl_out <- packages_data |>
  ...
  column_spec(2:ncol(packages_data), background = "#f0f8ff", border_left = TRUE, border_right = TRUE)
...
tbl_out <- sub("\\\\begin\\{table\\*\\}", "\\\\begin\\{table*\\}\\\\label\\{tbl-pkg-packages\\}", tbl_out)
...
# Set options for PDF table compatibility
...
knitr::kable(format_comp, 
  ...
  column_spec(1, border_left = TRUE, border_right = TRUE, color = "black", background = "white") %>%
  column_spec(2:ncol(format_comp), background = "#f0f8ff")
```
