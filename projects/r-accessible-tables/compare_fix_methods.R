...
packages_to_check <- c("data.table", "readr", "arrow", "polars")
...
file_size_mb <- file.size("test_data.csv") / (1024^2)
cat("CSV file size:", round(file_size_mb, 2), "MB\n")

# Also create compressed version
cat("Creating compressed version...\n")
write.csv(test_data, gzfile("test_data.csv.gz"), row.names = FALSE)
gz_size_mb <- file.size("test_data.csv.gz") / (1024^2)
...
results$base_read_csv <- simple_benchmark(quote(read.csv("test_data.csv")))
...
if ("data.table" %in% available_packages) {
  ...
  results$fread <- simple_benchmark(quote(fread("test_data.csv")))
  cat("data.table fread - Mean time:", round(results$fread$mean_time, 3), "seconds\n")
  
  # Test compressed CSV with fread
  cat("Testing fread with compressed CSV...\n")
  results$fread_gz <- simple_benchmark(quote(fread("test_data.csv.gz")))
...
if ("arrow" %in% available_packages) {
  ...
  results$arrow <- simple_benchmark(quote(read_csv_arrow("test_data.csv")))
...
if (file.exists("test_data.csv")) file.remove("test_data.csv")
if (file.exists("test_data.csv.gz")) file.remove("test_data.csv.gz")
